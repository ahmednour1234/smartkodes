@php
    ...
    $offsetClass = $isAdminRoute ? 'lg:ml-64' : 'md:ml-64';
    $toggleVisibilityClass = $isAdminRoute ? 'lg:hidden' : 'md:hidden';
@endphp

<div class="bg-white border-b border-gray-200 shadow-sm {{ $offsetClass }}">
                ...
                <details class="relative">
                    <summary class="list-none cursor-pointer inline-flex items-center gap-2 pl-2 pr-3 py-1.5 rounded-md border border-gray-200 bg-white hover:bg-gray-50">
                        @if($currentUser?->photo_url)
                            <img src="{{ $currentUser->photo_url }}" alt="{{ $displayName }}" class="h-8 w-8 rounded-full object-cover border border-gray-200">
                        @else
                            <div class="h-8 w-8 rounded-full border border-gray-200 bg-gray-100 text-gray-700 text-xs font-semibold flex items-center justify-center">
