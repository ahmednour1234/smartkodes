        ...
        <form method="POST" action="{{ route('tenant.projects.store') }}" id="projectForm" class="space-y-8">
          ...
          <section class="border-l-4 border-blue-500 pl-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label for="code" class="block text-sm font-medium text-gray-700 mb-2">Project Code <span class="text-red-500">*</span></label>
...
if (nameEl && codeEl) {
  nameEl.addEventListener('input', e => {
    if (!codeEl.value || codeEl.dataset.autogenerated === 'true') {
      const code = e.target.value
        .toUpperCase()
        .replace(/[^A-Z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '')
        .substring(0, 20);
      codeEl.value = code; codeEl.dataset.autogenerated = 'true';
    }
  });
  codeEl.addEventListener('input', () => { if (codeEl.value) delete codeEl.dataset.autogenerated; });
}
</script>
@endpush
@endsection
