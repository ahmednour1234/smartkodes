    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('tenant.users.update', $user) }}" enctype="multipart/form-data" class="p-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-6">
                        ...
                        @if($user->photo_url)
                            <img src="{{ $user->photo_url }}" alt="{{ $user->name }}" class="mb-3 h-16 w-16 rounded-full object-cover border border-gray-200">
                        @endif
                        ...
                        <p class="mt-1 text-sm text-gray-500">Optional. Uploading a new photo replaces the current one.</p>
                        @error('photo')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        @if($user->id_copy_url)
                            <a href="{{ $user->id_copy_url }}" target="_blank" class="mb-3 inline-block text-sm text-indigo-600 hover:text-indigo-800">View current file</a>
                        @endif
                        ...
                        <p class="mt-1 text-sm text-gray-500">Optional. Uploading a new file replaces the current one.</p>
                        @error('id_copy')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        @if($user->driving_license_url)
                            <a href="{{ $user->driving_license_url }}" target="_blank" class="mb-3 inline-block text-sm text-indigo-600 hover:text-indigo-800">View current file</a>
                        @endif
                    ...
                    <div class="mb-6">
                        ...
                        <select name="role_ids[]" id="role_id"
                            ...
                            @foreach($roles as $role)
                                <option value="{{ $role->id }}" {{ old('role_ids.0', $user->roles->first()?->id) == $role->id ? 'selected' : '' }}>
                                    {{ $role->name }}
                                </option>
                            @endforeach
                        </select>
