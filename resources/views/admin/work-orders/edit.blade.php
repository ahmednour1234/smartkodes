...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="mb-6">
        <div class="flex items-center text-sm text-gray-600 mb-2">
            ...
            <a href="{{ route('admin.work-orders.show', $workOrder->id) }}" class="hover:text-blue-600">Work Order Details</a>
            <i class="fas fa-chevron-right mx-2 text-xs"></i>
            <span class="text-gray-900 font-medium">Edit</span>
        </div>
        <h1 class="text-3xl font-bold text-gray-900">Edit Work Order</h1>
        <p class="text-gray-600 mt-1">Update work order details and assignment</p>
    </div>
                ...
                <div>
                    ...
                    <select id="project_id" name="project_id" required
                        ...
                        @foreach($projects as $project)
                            <option value="{{ $project->id }}" {{ (old('project_id', $workOrder->project_id) == $project->id) ? 'selected' : '' }}>
                                {{ $project->name }}
                            </option>
                        @endforeach
                    </select>
    ...
    <div class="mt-6 bg-red-50 border border-red-200 rounded-lg p-6">
        ...
        <form action="{{ route('admin.work-orders.destroy', $workOrder->id) }}" method="POST"
            ...
            <button type="submit" class="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-lg transition duration-200">
                <i class="fas fa-trash mr-2"></i>Delete Work Order
            </button>
        </form>
    </div>
</div>
