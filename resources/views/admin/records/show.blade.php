...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-6">
        <div class="flex justify-between items-start">
            <div>
                ...
                <p class="mt-2 text-sm text-gray-600">Viewing submission for {{ $record->form->name }}</p>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('admin.records.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500">
                    <svg class="inline h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                    </svg>
                    Back to List
                </a>
                <a href="{{ route('admin.records.edit', $record->id) }}" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <svg class="inline h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <div class="px-6 py-4 space-y-2">
                    <!-- Edit Record -->
                    @can('update', $record)
                    <a href="{{ route('admin.records.edit', $record->id) }}" class="flex items-center justify-center w-full px-4 py-2 bg-indigo-600 text-white text-center rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors">
                    ...
                    <!-- View Form Template -->
                    @if($record->form)
                    <a href="{{ route('admin.forms.show', $record->form->id) }}" class="flex items-center justify-center w-full px-4 py-2 bg-gray-600 text-white text-center rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 transition-colors">
                        <svg class="h-4 w-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                        </svg>
                        View Form Template
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden border-2 border-red-200">
                ...
                <div class="px-6 py-4">
                    ...
                    <form action="{{ route('admin.records.destroy', $record->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this submission? This action cannot be undone.');">
                        @csrf
                        @method('DELETE')
                ...
                <div class="px-6 py-4">
                    <div class="space-y-3">
                        ...
                        <div class="border rounded-lg p-4 {{ $approval->status === 'approved' ? 'border-green-300 bg-green-50' : ($approval->status === 'rejected' ? 'border-red-300 bg-red-50' : ($approval->status === 'delegated' ? 'border-blue-300 bg-blue-50' : 'border-yellow-300 bg-yellow-50')) }}">
                            ...
                            <!-- Approve Form (Hidden) -->
                            <form id="approve-form-{{ $approval->id }}" action="{{ route('admin.records.approve', [$record->id, $approval->id]) }}" method="POST" class="hidden mt-3 pt-3 border-t border-gray-300">
                                @csrf
                            ...
                            <!-- Reject Form (Hidden) -->
                            <form id="reject-form-{{ $approval->id }}" action="{{ route('admin.records.reject', [$record->id, $approval->id]) }}" method="POST" class="hidden mt-3 pt-3 border-t border-gray-300">
                                @csrf
    ...
    <div class="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            ...
            <div class="px-6 py-4">
                <!-- Add Comment Form -->
                <form action="{{ route('admin.records.add-comment', $record->id) }}" method="POST" class="mb-6">
                    @csrf
                    <div class="relative">
                        ...
                        <div class="flex items-start justify-between">
                            <div class="flex items-start flex-1">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <!-- Reply Form (Hidden) -->
                                    <form id="reply-form-{{ $comment->id }}" action="{{ route('admin.records.add-comment', $record->id) }}" method="POST" class="hidden mt-3">
                                        @csrf
                                        ...
                                        <div class="bg-white rounded-lg p-3">
                                            <div class="flex items-start">
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    ...
                                                    <form action="{{ route('admin.records.delete-comment', [$record->id, $reply->id]) }}" method="POST" class="inline-block mt-1" onsubmit="return confirm('Delete this reply?');">
                                                        @csrf
                                                        @method('DELETE')
                            ...
                            <form action="{{ route('admin.records.delete-comment', [$record->id, $comment->id]) }}" method="POST" onsubmit="return confirm('Delete this comment and all replies?');">
                                @csrf
                                @method('DELETE')
    ...
    <div id="assign-modal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            ...
            <form action="{{ route('admin.records.assign', $record->id) }}" method="POST">
                @csrf
                <div class="mb-4">
    ...
    <div id="approval-modal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            ...
            <form action="{{ route('admin.records.request-approval', $record->id) }}" method="POST">
                @csrf
                <div class="mb-4">
