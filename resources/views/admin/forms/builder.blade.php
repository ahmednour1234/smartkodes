        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-4">
                ...
                <div class="flex items-center space-x-3">
                    ...
                    @endif
                    <a href="{{ route('admin.forms.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                        ← Back to Forms
                    </a>
                </div>
            </div>
            ...
            <div class="col-span-3">
                <div class="bg-white rounded-lg shadow-sm border">
                    ...
                    <div class="p-4 space-y-2" id="field-palette">
                        ...
                        <div class="field-item" data-type="voice_message" draggable="true">
                            <div class="flex items-center p-3 border border-gray-200 rounded-md hover:border-blue-300 hover:bg-blue-50 cursor-move transition-colors">
                                <svg class="w-5 h-5 text-gray-400 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5a3 3 0 00-3 3v4a3 3 0 106 0V8a3 3 0 00-3-3zm5 7a5 5 0 01-10 0m5 5v3m-3 0h6"></path>
                                </svg>
                                <span class="text-sm font-medium text-gray-700">Voice Message</span>
                            </div>
                        </div>
...
<!-- Publish Form -->
<form id="publish-form" method="POST" action="{{ route('admin.forms.publish', $form) }}" style="display: none;">
    @csrf
    @method('POST')
</form>
...
(function() {
    ...
    function getDefaultFieldConfig(fieldType) {
        var configs = {
            ...
            voice_message: { label: 'Voice Message', required: false },
            file: { label: 'File Upload', required: false },
            gps: { label: 'GPS Location', required: false },
            barcode: { label: 'Barcode', placeholder: 'Or enter code manually', required: false }
        };
    ...
    function generateFieldPreview(fieldType, config) {
        switch (fieldType) {
            ...
            case 'voice_message':
                return '<div class="border border-dashed border-gray-300 rounded-md h-20 flex items-center justify-center text-gray-400 text-sm">🎤 Voice Message Upload</div>';
            case 'file':
            ...
            case 'barcode':
                return '<div class="space-y-1"><div class="border border-gray-300 rounded-md h-10 flex items-center justify-center text-gray-400 text-sm">📱 Scan Barcode</div><input type="text" placeholder="Or enter manually" class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm" disabled></div>';
            default:
    ...
    function showFieldProperties(fieldType, fieldId) {
        ...
        if (fieldType === 'text' || fieldType === 'textarea' || fieldType === 'email' || fieldType === 'phone' || fieldType === 'url' || fieldType === 'number' || fieldType === 'barcode') {
            html += '<div>' +
    ...
    document.getElementById('save-btn').addEventListener('click', function() {
        ...
        saveForm.action = "{{ route('admin.forms.save-builder', $form) }}";
